...
interface DeleteDiscountConfirmationModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: () => void;
    discountName: string;
...
const DeleteDiscountConfirmationModal: React.FC<DeleteDiscountConfirmationModalProps> = ({ isOpen, onClose, onConfirm, discountName }) => {
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
            <div className="bg-gray-900 rounded-lg p-6 max-w-sm w-full shadow-lg">
                ...
                <p className="text-gray-300 mb-6">
                    Tem certeza que deseja apagar a promoção <span className="font-bold text-white">"{discountName}"</span>? Esta ação não pode ser desfeita.
                </p>
