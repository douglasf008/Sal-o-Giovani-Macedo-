...
interface AdminChatbotFABProps {
    onOpen: () => void;
}

const AdminChatbotFAB: React.FC<AdminChatbotFABProps> = ({ onOpen }) => {
    ...
    useEffect(() => {
        if (fabRef.current) {
            ...
            setPosition({
                ...
                y: window.innerHeight - fabRect.height - 24,
            });
    ...
    return (
        <button
            ...
            className="bg-gradient-to-r from-teal-500 to-cyan-600 text-white p-4 rounded-full shadow-lg active:scale-95 active:cursor-grabbing transition-transform transition-opacity duration-200 z-40 cursor-grab opacity-75 hover:opacity-100"
            aria-label="Abrir assistente do funcionário"
        >
...
export default AdminChatbotFAB;