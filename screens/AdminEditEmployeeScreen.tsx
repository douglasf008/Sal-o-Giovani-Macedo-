...
const AdminEditEmployeeScreen: React.FC = () => {
    const navigate = useNavigate();
    const { employeeId } = useParams<{ employeeId: string }>();
    const { professionals, updateProfessional } = useProfessionals();
    const { services } = useServices();
    const { roles: availableRoles } = useRoles();
    const { loggedInProfessional, login } = useAuth();
    const { packageTemplates } = usePackages();

    const employeeToEdit = useMemo(() => professionals.find(p => p.id === employeeId), [professionals, employeeId]);
    ...
    useEffect(() => {
        if (employeeToEdit) {
            ...
            setSelectedServices(employeeToEdit.serviceIds);
            setSelectedPackageTemplates(employeeToEdit.packageTemplateIds || []);
            setSelectedClientBookablePackages(employeeToEdit.clientBookablePackageIds || []);
            setSelectedPermissions(employeeToEdit.permissions || []);
            setHasSpecificHours(!!(employeeToEdit.startTime || employeeToEdit.endTime || employeeToEdit.workingDays));
            setStartTime(employeeToEdit.startTime || '');
            setEndTime(employeeToEdit.endTime || '');
            setWorkingDays(employeeToEdit.workingDays || []);
            setAvatarPreview(employeeToEdit.avatarUrl);
            setEmploymentType(employeeToEdit.employmentType || 'commissioned');
            setFixedSalary(String(employeeToEdit.fixedSalary || ''));
            setRentValue(String(employeeToEdit.rentValue || ''));
            setIsSalaryActive(employeeToEdit.isSalaryActive ?? true);
            setSalarySource(employeeToEdit.salarySource || 'salon');
    ...
    const handleSave = () => {
        if (!name || !employeeToEdit) {
            alert('Erro: funcionário não encontrado ou nome inválido.');
            return;
        }
        ...
        const updatedProfessional: Professional = {
            ...
            avatarUrl: avatarPreview || `https://i.pravatar.cc/150?u=${name.replace(/\s/g, '')}`,
            employmentType,
            ...
            phone,
            email,
            startTime: hasSpecificHours ? startTime : undefined,
            endTime: hasSpecificHours ? endTime : undefined,
            workingDays: hasSpecificHours ? workingDays : undefined,
        };

        updateProfessional(updatedProfessional);
        
        // If the user is editing their own profile, update the auth state immediately.
        if (loggedInProfessional && loggedInProfessional.id === updatedProfessional.id) {
            login(updatedProfessional);
        }
    ...
    return (
        <div className="flex flex-col h-screen bg-black text-white">
            <header className="flex items-center p-4">
                ...
                <h1 className="text-xl font-bold mx-auto">Editar Funcionário</h1>
                            ...
                            <div className="border-t border-gray-800 pt-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {professionals
                                            .filter(p => p.id !== employeeToEdit?.id)
                                            .map(prof => (
